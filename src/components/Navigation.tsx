    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 font-sans">
      <nav className="bg-green-900/70 backdrop-blur-lg border border-green-800/50 rounded-full shadow-xl">
        <div className="flex items-center justify-between h-14 px-4 space-x-6">
          ...
          <a
            href="/"
            className="flex items-center justify-center w-10 h-10 rounded-full bg-black/20 group"
          >
            <img
              src={logo}
              alt="Logo"
          ...
          <div className="flex items-center space-x-1">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.path}
                className="text-sm font-medium px-4 py-2 rounded-full text-orange-200 hover:text-white transition-all duration-300"
              >
                {item.name}
              </a>
            ))}
